...
export function CardForm({ card, onSave }: CardFormProps) {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    if (card?.id) {
      ...
      const cardRef = doc(firestore, `users/${user.uid}/cards`, card.id);
      ...
      // Add new card
      const newCardId = doc(collection(firestore, `users/${user.uid}/cards`)).id;
      const cardRef = doc(firestore, `users/${user.uid}/cards`, newCardId);
